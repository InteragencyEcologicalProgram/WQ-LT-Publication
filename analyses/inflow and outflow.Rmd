...
dayflow <- readRDS(here("data/processed/dayflow.rds"))
...
# Calculate WYday for the first day of each month
wdoy_months <- dayflow_c %>%
  mutate(Month = month(Date, label = TRUE)) %>%
  group_by(Month) %>%
  summarise(day_min = min(WYday)) %>%
  mutate(month_label = day_min + 15)

# Create custom x-axis breaks and labels for the figures
wdoy_x_axis_spec <- list(
  scale_x_continuous(
    breaks = c(wdoy_months$day_min, wdoy_months$month_label), 
    labels = c(rep("", 12), as.character(wdoy_months$Month))
  )
)
```

...
```{r}
Am2 = lm(LogAm ~ Drought*Region+ Drought*Season + Region*Season + StocktonUpgrade, data = Ammonia)
summary(Am2)
plot(Am2)
...
Chl1 = lm(LogChl ~ Drought*Region+ Drought*Season + Region*Season, data = ChlaA)
summary(Chl1)
plot(Chl1)
library(car)
qqPlot(Chl1)
...
Yup. Chlorophyll still refuses to be interesting. It's also not quite meeting my linear regression assumptions. Am I allowed to take the square-root of a log? Probably not. Maybe a gamma distribution? Or just go to brms?

I need to get Sam to check my work on this. Is lognormal appropriate?
...
Well, if i did do that right, droughts are still not very interesting. Maybe chlorophyll is lower in Suisun Bay, but that's it. 

...
result_plotter<-function(contrast, variable, xlabel, log){
  ...
  if(log){
    contrasts<-contrasts%>%
      mutate(across(c(the.emmean, lcmpl, rcmpl), ~exp(.x)))
  }
  
  p<-ggplot(contrasts, aes(y=Drought, x=the.emmean))+
    geom_pointrange(aes(xmin=lcmpl, xmax=rcmpl))+
    facet_wrap(~.data[[variable]], scales="free_x", ncol=1)+
    xlab(xlabel)+
    theme_bw()
  
  diffs<-filter(contrasts, Drought%in%c("D", "W"))%>%
    select(Drought, all_of(variable), `the.emmean`)%>%
    pivot_wider(names_from = Drought, values_from = the.emmean)%>%
    mutate(difference=D-W)
...
# Region
result_plotter(m_t_contrasts_r, "Region", "Temperature (°C)", log=FALSE)

# Season
result_plotter(m_t_contrasts_s, "Season", "Temperature (°C)", log=FALSE)
```
...
```{r, fig.width=4, fig.height=6}
# Region
result_plotter(m_sa_contrasts_r, "Region", "Salinity", log=TRUE)

# Season
result_plotter(m_sa_contrasts_s, "Season", "Salinity", log=TRUE)
```
...
```{r, fig.width=4, fig.height=6}
# Region
result_plotter(m_se_contrasts_r, "Region", "Secchi depth (log)", log=TRUE)

# Season
result_plotter(m_se_contrasts_s, "Season", "Secchi depth (log)", log=TRUE)
```
...
```{r, fig.width=4, fig.height=6}
# Region
result_plotter(m_se2_contrasts_r, "Region", "Secchi depth (log)", log=TRUE)

# Season
result_plotter(m_se2_contrasts_s, "Season", "Secchi depth (log)", log=TRUE)
```

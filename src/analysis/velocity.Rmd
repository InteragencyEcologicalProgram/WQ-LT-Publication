...
```{r import data}
vel_weekly <- readRDS(here("data/processed/velocity.rds"))

lat_long <- read_csv(here("data/external/vel_coord.csv"))
```
...
```{r}
lat_long_sf <- st_as_sf(lat_long, coords = c("Longitude", "Latitude"), crs = 4326)

mapview(lat_long_sf, layer.name = "Velocity", size = 10, label = "site")
```

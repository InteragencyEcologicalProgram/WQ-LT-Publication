...
source(here("helperfunctions.R"))
source(here("analyses/pub_figure_funcs.R"))
```
...
#calculate means for the treatment plant upgrade by region
amRegime = emmeans(Am2, pairwise ~ StocktonUpgrade|Region)
amRegime
#put results in a better format and back transform
...
#calculate differences another way
diffsam2 = group_by(Ammonia, Region, StocktonUpgrade) %>%
  summarize(mean = mean(DissAmmonia, na.rm = T)) %>%
  group_by(Region) %>%
  summarize(diff = mean[1]-mean[2])
...
partial.r2(NatAn)
#partial R2 for drought is 0.127
...
#calculate effect size by regime
...
#calculate differences a different way
diffchl = group_by(ChlaA, Regime, Region) %>%
  summarize(Chl = mean(Chlorophyll, na.rm =T)) %>%
  group_by(Region) %>%
  summarize(DiffCHL = Chl[1]- Chl[2])

#now the good graph of everything and the effect sizes by region and season
...
save(ChRegion, CHSeason, NNRegion, NNSeason, OPRegion, OPSeason, AmRegions, AmSeasons, file = "../outputs/NutsCHLplotsandResults.RData")
```
...
anovas <-
  bind_rows(
    mutate(as_tibble(Aman, rownames = "Parameter"), Model = "Ammonia"),
    mutate(as_tibble(NatAn, rownames = "Parameter"), Model = "Nitrate + Nitrite"),
    mutate(as_tibble(Panova, rownames = "Parameter"), Model = "Orthophosphate"),
    mutate(as_tibble(Ch1_Anova, rownames = "Parameter"), Model = "Chlorophyll-a")
  ) %>%
  mutate(`Pr(>F)` = if_else(`Pr(>F)` < 0.001, "< 0.001", as.character(round(`Pr(>F)`, 4)))) %>%
  relocate(Model) %>%
  write_csv(here("outputs/Nutr_Chla_anovas.csv"))
...
tukeys <- 
  bind_rows(
    mutate(AmRegions$contrasts, Model = "Ammonia", `Contrast type` = "region"),
    mutate(AmSeasons$contrasts, Model = "Ammonia", `Contrast type` = "season"),
    ...
    mutate(OPRegion$contrasts, Model = "Orthophosphate", `Contrast type` = "region"),
    mutate(OPSeason$contrasts, Model = "Orthophosphate", `Contrast type` = "season"),
    mutate(ChRegion$contrasts, Model = "Chlorophyll-a", `Contrast type` = "region"),
    mutate(CHSeason$contrasts, Model = "Chlorophyll-a", `Contrast type` = "season")
  ) %>%
  rename(
    `p-value` = p.value, 
    `t-ratio` = t.ratio
  ...
  write_csv(here("outputs/Nutr_Chla_tukeys.csv"))

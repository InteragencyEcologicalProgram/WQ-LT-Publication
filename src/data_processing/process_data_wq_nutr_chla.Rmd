...
i_am("data_processing/process_data_wq_nutr_chla.Rmd")
```
...
df_regions <- read_csv(here("data_processing/region_assignments.csv"))
...
df_yr_type <- read_csv(here("data_processing/year_assignments.csv")) %>% rename(YearAdj = Year)
...
replace_blw_rl <- function(df, min_val = 0.00001, seed = 1) {
...
Export raw data for all parameters as .qs files. Also export, long-term average data both as .csv and .rds files for the analyses.
...
ls_data_raw %>% iwalk(\(x, idx) qsave(x, file = here("data", paste0(idx, ".qs"))))
...
ls_data_lt_avg %>% iwalk(\(x, idx) write_csv(x, file = here("data", paste0(idx, ".csv"))))

# Export long-term average data frames as rds files
ls_data_lt_avg %>% iwalk(\(x, idx) saveRDS(x, file = here("data", paste0(idx, ".rds"))))
```

...
p<-ggplot(d, aes(x=wdoy, y=InflowTotal_cum/InflowTotal_sum, color=Period_year2, group=WY, linetype=Period_year2))+
  geom_line()+
  geom_text_repel(data=filter(d, wdoy==120 & WY%in%c(1997, 1984)), aes(label=WY), max.overlaps=20, color="black")+
  facet_grid(Drought~., scales="free_y")+
  ...
  coord_cartesian(ylim=c(0, NA), expand = FALSE)+
  theme_bw()+
  theme(axis.ticks.x = element_line(color = c(rep("black", 12), rep(NA, 12))), panel.spacing = unit(1, "lines"))
p

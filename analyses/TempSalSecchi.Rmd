...
# Partial R2 from this post: https://stats.stackexchange.com/questions/64010/importance-of-predictors-in-multiple-regression-partial-r2-vs-standardized
...
```{r}
m_sa_contrasts_r <- pub_figure_plotter(df_data=wq_data, param=Salinity, y_label="Salinity", 
                                       fct_grp=Region, model=m_sa, log_trans = TRUE, plt_title = "Region")

m_sa_contrasts_r$diffs

m_sa_contrasts_s <- pub_figure_plotter(df_data=wq_data, param=Salinity, y_label="Salinity", 
                                       fct_grp=Season, model=m_sa, log_trans = TRUE, plt_title = "Season")
...
m_se_contrasts_r <- pub_figure_plotter(df_data=wq_data, param=Secchi, y_label="Secchi depth (cm)", 
                                       fct_grp=Region, model=m_se, log_trans = TRUE, plt_title = "Region")

m_se_contrasts_r$diffs

m_se_contrasts_s <- pub_figure_plotter(df_data=wq_data, param=Secchi, y_label="Secchi depth (cm)", 
                                       fct_grp=Season, model=m_se, log_trans = TRUE, plt_title = "Season")
...
m_se2_contrasts_r <- pub_figure_plotter(df_data=wq_data, param=Secchi, y_label="Secchi depth (cm)", 
                                        fct_grp=Region, model=m_se2, log_trans = TRUE, plt_title = "Region")

m_se2_contrasts_r$diffs

m_se2_contrasts_s <- pub_figure_plotter(df_data=wq_data, param=Secchi, y_label="Secchi depth (cm)", 
                                        fct_grp=Season, model=m_se2, log_trans = TRUE, plt_title = "Season")
...
anovas<-bind_rows(
  ...
  mutate(`Pr(>F)`=if_else(`Pr(>F)`<0.001, "< 0.001", as.character(formatC(signif(`Pr(>F)`, 4), digits=4, format="fg", flag="#"))),
         ...
         across(c(`Sum Sq`, `F value`, `Pr(>F)`), ~paste0(ifelse(str_detect(.x, fixed("NA")) | is.na(.x), "", .x), "ZZZ")))%>%
  relocate(model)%>%
...
tukeys<-bind_rows(
  ...
  mutate(`p-value`=if_else(`p-value`<0.001, "< 0.001", as.character(formatC(signif(`p-value`, 4), digits=4, format="fg", flag="#"))),
         ...
         # Excel is the worst, I had to add letters to these numbers so it wouldn't re-round them
         across(c(estimate, SE, `t-ratio`, `p-value`), ~paste0(ifelse(str_detect(.x, fixed("NA")) | is.na(.x), "", .x), "ZZZ")))%>%
  select(Model=model, `Contrast type`=`contrast type`, Group, Contrast=contrast,
         Estimate=estimate, SE, DF=df, `t-ratio`, `p-value`)%>%
